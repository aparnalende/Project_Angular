...
export class SigninComponent implements OnInit {
  ...
  onSubmit() {
    ...
    if (
      (this.signinUser.username === '' || this.signinUser.username === null) &&
      (this.signinUser.password === '' || this.signinUser.password === null)
    ) {
      Swal.fire('Please fill out the given fields!!', 'error');
    } else {
      this.servObj.getUserUsingUsername(this.signinUser.username).subscribe(
        (data) => {
          this.user = data;
